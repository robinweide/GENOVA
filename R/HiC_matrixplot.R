...
#plot the CTCF sites as arrow heads
...
#next step in simplyfying is to make it possible to add an arbitrary amount of features
#using a list of chip data
...
#install_github("jayhesselberth/bigwrig")
plot.bw <- function( file, chrom, start, end, y1, y2, col, rotate=F){
	...
	d <- read_bigwig_impl( file, chrom=chrom, start=start, end=end)
...
#exp2 = treatment (top)
#chrom = chromosome
#start,end = start and end position of the region of interest
#cut.off = the cut.off for the hic-matrix plot, in the diff option the negative of this is the lower bound
#chip = a list of feature tracks, can be bed structure (i.e. data frames) or a path to bigwig file (i.e. character variable), maximum length of 4, first two and last two have to be the same (i.e. 1:bed,2:bed,3:path,4:path, but 1:bed,2:NULL,3:path,4:NULL is also allowed)
#bed.col = color of the bed track (max.len is 4)
#bw.col = same as bed col, but for the bigwig track
#type = should a rectangle or a triangle be drawn? Not that for a triangle a 6th strand column should be included
#coplot = when drawing together two experiments, dual is bottom triangle exp1, top triangle exp2; diff plots a substraction of exp2-exp1
#genes = structure with gene information, will only be combined with bed structure
