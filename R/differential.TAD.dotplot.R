...
differential.TAD.dotplot <- function( exp1, exp2, color.fun=NULL, yRange =NULL, pch = '.', title = NULL, cex = 1, ... ){
  ...
  yLab = paste0('log2 contact frequency ( ', exp2$sampleName, " / ",  exp1$sampleName," )")
  #plot( tad.dist + runif(nrow(comb.exp), -0.4, 0.4), log.ratio, pch='.', main = theTitle, col=col.vec[tad.dist], ylab="Log2-ratio contact frequency", xlab="TAD distance", xaxt='n', ...)
