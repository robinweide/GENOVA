...
#' @param focus Sort on which sample? (give the index-number)
...
#' @param profileFunct Function to make profile-plots (mean, median, sum, absSum)
#' @param flank Amount of flanking bp.
#' @param zlim zlims c(min,max)
#' @param profileCols Vector of colors for profiles
#' @param profileZlim zlims c(min,max) for profiles
...
#' @return A plot
#' @export
insulation.heatmap <- function(insulationList, bed,  focus = 1, sortWidth = 10, whatToPlot = 'both',  profileFunct = mean, flank=500e3, zlim = c(-1,1), profileCols = NULL, profileZlim = NULL, heatmapCols = c("#f03b20", "#ffeda0", "white",  "#31a354"),  rmNonVariateRows = T, verbose = F ){


  if(!whatToPlot %in% c('profile', 'heatmap', 'both')){stop('whatToPlot should be "profile", "heatmap" or "both".')}
  ...
  if(rmNonVariateRows){
    nonVariateRows = as.numeric(names(table(NVrows))[table(NVrows) == length(insulationList)])
  ...
  if(is.null(profileZlim)){
    ...
    PRFLS <- ggplot2::ggplot(profDF, ggplot2::aes(Var1, val, col = sample)) +
      ggplot2::geom_line()  +
      ggplot2::scale_x_continuous(expand=c(0,0), breaks = tickPoss, labels = tickLabs) +
      ...
      ggplot2::theme(panel.grid.minor = ggplot2::element_blank(),axis.title.x=ggplot2::element_blank())
  } else {
    PRFLS <- ggplot2::ggplot(profDF, ggplot2::aes(Var1, val, col = sample)) +
      ggplot2::geom_line() +
      ggplot2::scale_x_continuous(expand=c(0,0), breaks = tickPoss, labels = tickLabs) +
      ...
      ggplot2::scale_colour_manual(values = profileCols) +
      ggplot2::theme(aspect.ratio = 1) +
      ggplot2::facet_grid(. ~ sample ) + ggplot2::theme(panel.grid.minor = ggplot2::element_blank(),
  ...
  HTMPS <- ggplot2::ggplot(df, ggplot2::aes(Var1, Var2, fill = value)) +
    ...
    #ggplot2::scale_fill_distiller(type = 'seq', palette = brewerPal, direction = brewerDir )+
  ...
  if(whatToPlot == 'profile'){
    ...
    if(is.null(profileZlim)){
      ...
      PRFLS <- ggplot2::ggplot(profDF, ggplot2::aes(Var1, val, col = sample)) +
        ggplot2::geom_line()  +
        ggplot2::scale_x_continuous(expand=c(0,0), breaks = tickPoss, labels = c("","","")) +
        ggplot2::scale_y_continuous(expand=c(0,0), limits = profileZlim ) +
        ggplot2::labs( x = '', y = '') + ggplot2::guides(col = F) +
        ggplot2::scale_colour_manual(values = profileCols)+
        ggplot2::theme(aspect.ratio = 1) +
      ...
      PRFLS <- ggplot2::ggplot(profDF, ggplot2::aes(Var1, val, col = sample)) +
        ...
        ggplot2::scale_x_continuous(expand=c(0,0), breaks = tickPoss, labels = c("","","")) +
        ggplot2::scale_y_continuous(expand=c(0,0), limits = profileZlim ) +
        ggplot2::labs( x = '', y = '') + guides(col = F) + ggplot2::theme_linedraw()+
        ggplot2::scale_colour_manual(values = profileCols) +
        ggplot2::theme(aspect.ratio = 1) +
        ggplot2::facet_grid(. ~ sample ) + ggplot2::theme(plot.margin = ggplot2::unit(c(0, 0, 0, 0), "cm")) +
        ggplot2::theme(panel.grid.minor = ggplot2::element_blank(),axis.title.x=ggplot2::element_blank())
    }


    grid::grid.newpage()
    grid::grid.draw(rbind(ggplot2::ggplotGrob(PRFLS), ggplot2::ggplotGrob(HTMPS),
                          size = "last"))
    invisible(df)
  }
