...
selectData_saddle <- function(exp, chrom1, chrom2) {
  bed <- exp$IDX
  data <- exp$MAT
  X <- bed[bed[, V1] == chrom1, V4]
  Y <- bed[bed[, V1] == chrom2, V4]
  ...
  x.pos <- bed[bed[, V1] == chrom1, V2]
  y.pos <- bed[bed[, V1] == chrom2, V2]
...
saddle.core <- function(exp, chip, chrom, start, end, CS, nBins) {
  ...
  for (i in 1:nBins) {
    for (j in 1:nBins) {
      sel.x <- which(bins == unique.bins[i])
      sel.y <- which(bins == unique.bins[j])
...
visualise_saddle <- function(SBoutList, addText = T,
  ...
  for (Si in 1:NSAMPLE) {
    ...
    tmp <- setNames(
      aggregate(tmp$value, by = list(tmp$Var1, tmp$Var2), mean, na.rm = T),
      c("x", "y", "z")
    )
    ...
    image(M,
      zlim = zlim, col = PAL(50), axes = F,
      breaks = 2**(seq(log2(zlim[1]), log2(zlim[2]), length.out = 51))
    )
    box()
