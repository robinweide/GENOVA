...
zzzzRCP <- function(experimentList, chromsToUse = NULL, bedList = NULL, colors = NULL, maxDistance = NULL, ignoreLengthWarning = F, outlierCutoff = 1, verbose = F) {
  ...
  if (length(unique(RESSES)) != 1) {
    warning("The resolutions of the different experiments are not the same")
  }
  ...
  if (length(chromsToUse) > 50) {
    stop("Please restrict the amount of chromosomes with chromsToUse")
  }
...
"))
      ...
      if (!is.null(bedList)) {
        if (!is.list(bedList)) {
          stop("bedList must be a named list of dataframes!")
        }
...
coreRCP <- function(m.chrom, resolu, breaks, outlierCutoff, bedi = NULL) {
  ...
  rcp <- tapply(m.chrom$V3, cbb, FUN = function(x) {
    outlierRemovedMean(x, outlierCutoff)
  })
