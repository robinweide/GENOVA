...
visualise_APA_ggplot <- function(APAlist, title = "APA", zTop = NULL, zBottom = NULL, focus = 1, ...) {
  ...
  belownames <- vector()
  for (i in 1:list.len) {
    firstMat <- as.data.frame(APAlist[[i]]$APA)
    colnames(firstMat) <- 1:size
    rownames(firstMat) <- 1:size
    secondMat <- as.data.frame(APAlist[[focus]]$APA)
    ...
    e$sample <- rep(paste0(
      names(APAlist)[focus], " vs ",
      names(APAlist)[i]
    ), length(e[, 1]))
    belowPlots <- rbind(belowPlots, e)
    belownames <- c(belownames, paste0(
      names(APAlist)[focus],
      " vs ", names(APAlist)[i]
    ))
  }
  ...
  if (is.null(zTop)) {
    z <- c(
      quantile(na.exclude(abovePlots$value), 0.01),
  ...
  abovePlots$sample <- factor(abovePlots$sample, levels = c(
  ...
  plot1 <- ggplot2::ggplot(abovePlots, ggplot2::aes(Var1, Var2)) +
    ...
    ggplot2::scale_x_continuous(
      breaks = c(tickPosDownstream, size.banks + 1, tickPosUpstream),
      expand = c(0, 0),
  ...
  if (is.null(zBottom)) {
    z2 <- c(
      quantile(na.exclude(belowPlots$value), 0.01),
