...
visualise_RCP_ggplot <- function(RCPdata, smooth = F, combine = T, ylim = NULL, xlim = NULL, lineOnly = T, lineWidth = 1, errWidth = 1, pointWidth = 0.5) {
  ...
  if (smooth == F) {
    if (colorOnBed) {
      p <- ggplot2::ggplot(RCPdata, ggplot2::aes(
        col = BED, x = distance / 1000000,
        y = prob
      )) +
        ...
        ggplot2::labs(
          title = "Relative contact probability",
  ...
  suppressMessages(p + ggplot2::theme(
