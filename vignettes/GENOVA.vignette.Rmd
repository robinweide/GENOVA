...
## Loading data
```{r loading, eval = T, message=FALSE, cache.lazy=F, cache=T}
...
```{r stackedTAD, eval=T, fig.align='center', fig.width=9, message=FALSE, cache=T, cache.lazy=F}
# # Run stackedTAD
stackedTAD.dat.WT <- stackR(experiment = WT,tad.bed = hicsegBEDPE, verbose = F)
stackedTAD.dat.DKO <- stackR(experiment = DKO,tad.bed = hicsegBEDPE, verbose = F)

# Create list of stackedTAD-results
stackedTADResultList <- list(WT = stackedTAD.dat.WT,DKO = stackedTAD.dat.DKO)

image(stackedTAD.dat.WT$STACK)
image(stackedTAD.dat.DKO$STACK)
```

```{r stackedTADold, eval=F, fig.align='center', fig.width=9, message=FALSE, cache=T, cache.lazy=F}
# # Run stackedTAD
stackedTAD.old.dat.WT <- stackR.old(experiment = WT,tad.bed = hicsegBEDPE, verbose = F)
stackedTAD.old.dat.DKO <- stackR.old(experiment = DKO,tad.bed = hicsegBEDPE, verbose = F)

# Create list of stackedTAD-results
stackedTADResultList.old <- list(WT = stackedTAD.old.dat.WT,DKO = stackedTAD.old.dat.DKO)
```

```{r stackedTADvis, eval=F, fig.align='center', fig.width=5,fig.height=5, message=FALSE, cache=T, cache.lazy=F}
# Visualise with ggplot
visualise.stackR.ggplot(stackedTADResultList.old, focus = 1, title = 'Comparing WT and DKO: stacked TAD old')
