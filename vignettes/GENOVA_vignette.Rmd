...
### regions
```{r RCPBED, message=FALSE,   fig.small= T , fig.cap= "RCP with BEDs. We can also add BEDs as sites to compute the RCP. A-compartments have more contacts closerby."}
CTCF = read.delim('data/CTCF_WT_motifs.bed', h = F)
SMC1 = read.delim('data/SMC1_WT_peaks.narrowPeak', h = F)

SMC1_shifted = CTCF
SMC1_shifted[,2:3] = SMC1_shifted[,2:3] + sample(0:2e6, nrow(SMC1_shifted) )

RCP_out = RCP(experimentList = list(Hap1_WT_40kb ), 
               bedList =  list("CTCF" = CTCF, 
                               
                               'Shuffled' = CTCF[1:10e2,]), 
               chromsToUse = c('chr10'))
...
cis.compartment.plot(exp = Hap1_WT_40kb, chrom = 'chr20', arm = 'q',zlim = 20)
```
...
cis.compartment.plot(exp = Hap1_WT_40kb, chrom = 'chr20', arm = 'q',zlim = 1, obs.exp = T)
```
...
trans.compartment.plot(exp = Hap1_WT_40kb, chrom1 = 'chr9', arm1 = 'q', chrom2 = 'chr22', arm2 = 'q', zlim = 10)
...
### Insulation-heatmap
```{r INSalign, message=FALSE , fig.asp=1,fig.cap= "Insulation heatmap.",cache=T, fig.retina=T, dev = 'png', dpi=300,fig.small = F}
...
knitr::kable(
  head(WT_Loops[,1:6], 5), caption = "A data.frame holding a standard BEDPE format. Columns 1-3 are describe the 5' anchor, columns 4-6 describe the 3' anchor."
)
```
